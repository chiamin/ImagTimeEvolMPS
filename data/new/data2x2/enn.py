...
if __name__ == "__main__":
    data0 = readE0("2/init.dat")
    skip_steps = 100

    fnames = ["ntau50.dat","ntau100.dat","ntau150.dat","ntau200.dat"]#,"ntau50.dat"]#,"ntau60.dat","ntau70.dat","ntau80.dat"]
    Ntaus = [0, 50, 100, 150, 200]
    taus = np.array(Ntaus) * data0["dtau"]  # taus in the original code

    Ekst, errEkst, EVst, errEVst, Est, errEst, signst, err_signst = [data0["Ek0"]],[0.],[data0["EV0"]],[0.],[data0["E0"]],[0.],[1.],[0.]
    data = dict()
    for fil in fnames:
        for fdi in range(1,11):
            fname = str(fdi)+"/"+fil
            print(fname)
            if len(data) == 0:
                data = ana.read_monte_carlo_file(fname, skip_steps=skip_steps)
            else:
                tmp = ana.read_monte_carlo_file(fname, skip_steps=skip_steps)
                for k in data:
                    data[k] = np.concatenate((data[k], tmp[k]))
