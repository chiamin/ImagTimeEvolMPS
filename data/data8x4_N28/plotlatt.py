...
import ast  # safer than eval for parsing Python literals like lists
...
def read_nup_ndn(filename, key1, key2):
    nup = None
    ndn = None
    with open(filename, 'r') as f:
        for line in f:
            if line.startswith(key1):
                nup = np.array(ast.literal_eval(line.split(key1)[1].strip()))
            elif line.startswith(key2):
                ndn = np.array(ast.literal_eval(line.split(key2)[1].strip()))
    return nup, ndn

def myplot(nup, ndn, lx, ly, resize=2):
    f,ax = gen_figure (resize*lx, resize*ly)
...
if __name__ == "__main__":
    ...
    with PdfPages('hsz.pdf') as pdf:
        # Ground state
        nup, ndn = read_nup_ndn("init.dat","nup_GS","ndn_GS")
        f, ax = myplot(nup, ndn, lx, ly)
        ax.set_title("ground state")
        pdf.savefig(f)
        plt.close(f)
        # Initial state
        nup, ndn = read_nup_ndn("init.dat","nup0","ndn0")
        f, ax = myplot(nup, ndn, lx, ly)
        ax.set_title("ntau0")
        pdf.savefig(f)
        plt.close(f)
