...
function getProbWeightRot(conf::Vector{Int}, mps, phi_up::Matrix{T}, phi_dn::Matrix{T}, U_up::Matrix{T}, U_dn::Matrix{T}) where T
    o1 = detProdRotOverlap(phi_up, phi_dn, conf, U_up, U_dn)
...
function sampleBondRot(conf::Vector{Int}, mps, phi_up::Matrix{T}, phi_dn::Matrix{T}, bond::Vector{Int}, U_up::Matrix{T}, U_dn::Matrix{T}) where T
    i1, i2 = bond
    ...
    for conf_i in confs
        odet, omps = getProbWeightRot(conf_i, mps, phi_up, phi_dn, U_up, U_dn)
        wi = odet * omps
        push!(ws, wi)
        push!(OMPSs, omps)
    end
...
function sampleMPSRot!(conf::Vector{Int}, psi, phi_up::Matrix{T}, phi_dn::Matrix{T}, latt, U_up::Matrix{T}, U_dn::Matrix{T}) where T
    O = 0.
    for bond in latt.bonds
        conf, O = sampleBondRot(conf, psi, phi_up, phi_dn, bond, U_up, U_dn)
    end
    return conf, O
end
