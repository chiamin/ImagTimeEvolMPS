...
function run(Lx, Ly, tx, ty, xpbc, ypbc, Nup, Ndn, U, dtau, nsteps, N_samples, mps, write_step, dir, U_up, U_dn)
    Nsites = Lx*Ly
    Npar = Nup+Ndn
    tau = dtau * nsteps
...
function transform_Hubbard_basis_real_NN(Uup::Matrix{Float64}, Udn::Matrix{Float64},
                                              Lx::Int, Ly::Int, tx::Float64, ty::Float64,
                                              U::Float64, periodic_x::Bool=false, periodic_y::Bool=false)
    N = Lx * Ly
    Hnew = OpSum()

    # === One-body terms (nearest neighbor hopping only) ===
    @inbounds for xi in 1:Lx, yi in 1:Ly
        i = mix_ind(xi, yi, Lx, Ly)

        # x-direction
        if xi < Lx || periodic_x
            xj = (xi == Lx) ? 1 : xi + 1
            j = mix_ind(xj, yi, Lx, Ly)
            @views uupi = Uup[i, :]
            @views uupj = Uup[j, :]
            @views udni = Udn[i, :]
            @views udnj = Udn[j, :]
            for m in 1:N, n in 1:N
                t1 = -tx * (uupi[m] * uupj[n] + uupj[m] * uupi[n])
                if abs(t1) > 1e-12
                    Hnew += t1, "Cdagup", m, "Cup", n
                end
                t2 = -tx * (udni[m] * udnj[n] + udnj[m] * udni[n])
                if abs(t2) > 1e-12
                    Hnew += t2, "Cdagdn", m, "Cdn", n
                end
            end
        end
        ...
        if yi < Ly || periodic_y
            yj = (yi == Ly) ? 1 : yi + 1
            j = mix_ind(xi, yj, Lx, Ly)
            @views uupi = Uup[i, :]
            @views uupj = Uup[j, :]
            @views udni = Udn[i, :]
            @views udnj = Udn[j, :]
            for m in 1:N, n in 1:N
                t1 = -ty * (uupi[m] * uupj[n] + uupj[m] * uupi[n])
                if abs(t1) > 1e-12
                    Hnew += t1, "Cdagup", m, "Cup", n
                end
                t2 = -ty * (udni[m] * udnj[n] + udnj[m] * udni[n])
                if abs(t2) > 1e-12
                    Hnew += t2, "Cdagdn", m, "Cdn", n
                end
            end
    ...
    # === Two-body interaction terms (vectorized outer product method) ===
    # Precompute outer products for Uup and Udn:
    @inbounds for m in 1:N, n in 1:N
        up_vec = Uup[:, m] .* Uup[:, n]  # size N
        for p in 1:N, q in 1:N
            dn_vec = Udn[:, p] .* Udn[:, q]
            coef = U * dot(up_vec, dn_vec)
            if abs(coef) > 1e-12
                Hnew += coef, "Cdagup", m, "Cup", n, "Cdagdn", p, "Cdn", q
            end
        end
    end
...
function main()
    ...
    dims = [80,80,80,80,160,160,160,160,320,320,320,320,640,640,640,640]
    ...
    Cup = correlation_matrix(psi_GS, "Cdagup", "Cup")
    Cdn = correlation_matrix(psi_GS, "Cdagdn", "Cdn")
    eig_up = eigen(Cup)
    eig_dn = eigen(Cdn)
    U_up = eig_up.vectors
    U_dn = eig_dn.vectors
